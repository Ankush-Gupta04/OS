...
int main(){
	...
	char *final=input;
        ...
        // printf("%s",exection[0]);
	int check  = 0;else if (strcmpp(exection[0],"date") == 0 && strcmp(exection[1],"-I\n")){
                check = 1;
                strcat(rf,"/cat");
                execv(rf,"-I");
            }
        ...
        if (fork() == 0){
            if (strcmp(exection[0],"ls\n") == 0){
                ...
                check = 1;
                strcat(rf,"/cat");
                if (strcmp(exection[1],"-n") == 0){
                    execv(rf,exection[2],"-n");
                }
                else{
                     execv(rf,exection[1],NULL);
                }
            }
            else if (strcmpp(exection[0],"date") == 0 && strcmp(exection[1],"-I\n")){
                check = 1;
                strcat(rf,"/cat");
                execv(rf,"-I");
            }
            else if (strcmpp(exection[0],"date") == 0 && strcmp(exection[1],"-R\n")){
                check = 1;
                strcat(rf,"/cat");
                execv(rf,"-R");
            }
